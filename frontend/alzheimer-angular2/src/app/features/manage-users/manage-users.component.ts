...
export type UserTab = 'all' | 'patients' | 'staff' | 'caregivers';

export interface DirectoryUser {
  id: string;
  code?: string; // patientCode or username
  name: string;
  email: string;
...
export class ManageUsersComponent implements OnInit {
  ...
  // Real patient and staff data from services
  private realPatients = signal<DirectoryUser[]>([]);
  private realStaff = signal<DirectoryUser[]>([]);
  loading = signal(true);
  ...
  readonly users = computed(() => {
    const all = [...this.realPatients(), ...this.realStaff(), ...this.staticUsers];
  ...
  readonly tabs = [
    ...
    { id: 'staff' as UserTab, label: 'Staff' },
  ...
  readonly stats = computed(() => [
    ...
    },
    {
      ...
      value: (this.realStaff().length + this.staticUsers.length).toLocaleString(),
      trend: '0%',
      trendUp: null,
      icon: 'pi-heart',
  ...
  readonly meterData = computed(() => [
    ...
    { label: 'Staff', color: '#10b981', value: this.realStaff().length, icon: 'pi pi-id-card' },
  ...
  ngOnInit(): void {
    ...
    // Reload when data is refreshed elsewhere
  ...
  loadStaff(): void {
    this.staffService.getAll().subscribe({
      next: (data: StaffProfileResponse[]) => {
        const mapped: DirectoryUser[] = data.map((s, index) => ({
          id: s.id.toString(),
          code: s.username,
          name: s.fullName,
          email: `${s.username}@care.com`,
          role: 'Staff',
          status: s.active ? 'Active' : 'Offline',
          lastActivity: 'Active',
          avatar: `https://randomuser.me/api/portraits/men/${(index + 32) % 95}.jpg`,
          avatarBg: this.getRandomColor(s.id)
        }));
        this.realStaff.set(mapped);
      },
      error: (err) => console.error('Staff API Error:', err)
    });
  }
